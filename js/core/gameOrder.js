(function(host, name, $, Event) {
    ...
    var pros = {
        ...
        submitOrder: function(compress, data) {
            ...
            if (compress) {
                data = {
                    'compressedBetData': encodeURIComponent(that.getCompressData(data))
                };
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    } else if (response.errno == 3004) {
                        window.location.href = "/view/page/login.shtml";
                    } else {
                         Games.getCurrentGame().fireEvent('afterSubmitError', response);
                    }
                    //请求解锁
                    that.cancelPostLock();
                },
